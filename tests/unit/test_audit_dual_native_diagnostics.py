...
from benchmark_tools import audit_dual_native_diagnostics as module
from tests.unit.test_verify_dual_native_provenance import fixture, RESULTS
...
def panel(tmp_path, monkeypatch):
    ...
    for index in range(3):
        ...
        measured.update(native=dict(exit_code=0, timed_out=False, started_ns=(index * 10 + 1) * 10**9,
                        ...
                        points=[dict(frontier=dict(boot_id="boot"))])
        ...
        for name, value in (("preparation.json", prepared), ("verification.json", verified),
                            ("dual_diagnostic_task.json", receipt), ("measurement/dual_bracket_report.json", measured)):
...
def test_live_job_stops_before_reading_native_archive(panel, monkeypatch):
    path = panel[2] / "scheduler_21914.txt"
...
def test_missing_scheduler_stops_before_native_read(panel, monkeypatch):
    (panel[2] / "scheduler_21914.txt").unlink()
...
def test_failed_job_is_retained(panel):
    path = panel[2] / "scheduler_21913.txt"
...
def test_missing_native_report_does_not_exclude_task(panel):
    (panel[0] / "dual_bracket_native_v1/run_01/measurement/dual_bracket_report.json").unlink()
    result = module.audit(*panel)
    assert result["validated_tasks"] == 2
...
def test_changed_clock_domain_reported(panel):
    directory = panel[0] / "dual_bracket_native_v1/run_01"
    path = directory / "measurement/dual_bracket_report.json"
    measured = json.loads(path.read_text())
    measured["points"][0]["frontier"]["boot_id"] = "different"

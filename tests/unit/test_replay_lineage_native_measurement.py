...
from benchmark_tools import replay_lineage_native_measurement as module
from tests.unit.test_measure_native_lineage_step import extend_lineage as prepare
...
@pytest.mark.parametrize("fault", ["command", "cadence", "job", "exit", "timeout", "wall",
                                  ...
                                  "lineage", "point_gap", "point_extra", "raw_disagreement"])
def test_rejects_inconsistent_or_invalid_evidence(archive, fault):
    ...
    if fault == "job":
        ...
        for key in ("identities_before", "identities_after"):
            measured["points"][1]["lineage"][key]["/"][1] += 1
    write(archive)
...
def test_explicit_control_timeout_preserves_original_default(archive):
    ...
    assert result["status"] == "lineage_native_measurement_replayed"

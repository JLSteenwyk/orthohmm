...
@pytest.mark.parametrize("problem", [None, "status", "command", "postflight", "raw_job", "scored", "provenance"])
def test_execution_identity(problem):
    ...
    if problem == "status":
        ...
        status["provenance"] = {}
    args = (status, pre, post, cell, arm, {"JobIDRaw": "21317"}, 0, Path("/launcher"))
    if problem:
        with pytest.raises(ValueError):

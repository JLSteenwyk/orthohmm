...
from benchmark_tools import verify_lineage_native_provenance as module
...
def fixture(index):
    ...
    runtime.append(dict(path=str(module.ROOT / "lineage_native_recipe_v1.json"), sha256=module.RECIPE_SHA,
    ...
    measured = dict(job_id=module.JOBS[index], launched=["srun", "--exclusive", "--exact", "--nodes=1", "--ntasks=1",
        ...
        str(module.RECIPE_ROOT / "benchmark_tools/measure_native_lineage_step.py"), "--worker", run["measurement_directory"]])
    ...
    scheduler = (f"JobId={module.JOBS[index]} JobState=COMPLETED ExitCode=0:0 Restarts=0 Requeue=0 "
        ...
        f"Command={module.SUBMISSION_SCRIPT}")
...
def test_remote_recipe_path_is_not_the_submitted_script():
    args = fixture(0)
    args[-1] = args[-1].replace(str(module.SUBMISSION_SCRIPT),
        str(module.RECIPE_ROOT / "benchmark_tools/run_dgx_lineage_native.sh"))
    with pytest.raises(ValueError, match="Scheduler identity"):
        module.verify(*args)


@pytest.mark.parametrize("job", [21990, 21991, 21992])
def test_original_failed_attempt_cannot_replace_corrected_task(job):
    args = fixture(0)
    args[-1] = args[-1].replace(f"JobId={module.JOBS[0]}", f"JobId={job}")
    with pytest.raises(ValueError, match="Scheduler identity"):
        module.verify(*args)
...
def test_changed_provenance_rejected(fault):
    ...
    if fault == "receipt":
        ...
        args[-1] += " ArrayJobId=21997 ArrayTaskId=0"
    else:
        before, after = {"scheduler_job": ("JobId=21997", "JobId=21995"),
...
@pytest.mark.parametrize("name", ["dgx_lineage_native_plan_20260919.json", "dgx_lineage_native_recipe_20260919.json",
...
def test_context_pins_source_bytes(tmp_path, name):
    for source in ("dgx_lineage_native_plan_20260919.json", "dgx_lineage_native_recipe_20260919.json",
                   "dgx_pressure_overhead_plan_v2_20260919.json", "LINEAGE_NATIVE_PROTOCOL_20260919.md"):

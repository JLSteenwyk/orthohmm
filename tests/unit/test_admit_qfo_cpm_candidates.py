...
def test_scheduler(problem):
    row = ["21967_0", "21968", "COMPLETED", "0:0", "00:10:00", "bizon", "2"]
    if problem == "running":
        row[2] = "RUNNING"
    elif problem == "exit":
        row[3] = "1:0"
    ...
    if problem:
        ...
        assert module.completed_task(text, 0)["JobIDRaw"] == "21968"
...
def test_orchestration(tmp_path, monkeypatch, problem):
    def write(path, value):
        ...
        return record(path)
    root = tmp_path
    executor = root / "benchmarks/work/publication_qfo_cpm_candidates_v2"
    ...
    write(root / "benchmarks/work/qfo_cpm_candidates_21967_0.time.txt", {})

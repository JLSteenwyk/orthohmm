...
from benchmark_tools import run_dual_native_diagnostic as module
...
def test_frozen_plan_and_relocated_outputs():
    plan = build()
    assert json.loads((FOLDER / "dgx_dual_native_plan_20260919.json").read_text()) == plan
    ...
    for i, row in enumerate(plan["runs"]):
        ...
        assert "dual_bracket_native_v1" in row["run"]["measurement_directory"]

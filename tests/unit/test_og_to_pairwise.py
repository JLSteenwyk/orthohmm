...
def test_ambiguous_accession_rejected_even_within_species(tmp_path):
    (tmp_path / "one.fasta").write_text(
        ">sp|A|ONE\nAAAA\n>tr|A|TWO\nAAAA\n"
    )
    with pytest.raises(ValueError, match="ambiguous"):
        og_to_pairwise.index_genes(tmp_path)


def test_full_id_and_accession_cannot_double_count_gene(tmp_path, monkeypatch):
    input_dir = tmp_path / "input"
    _write_input(input_dir)
    groups = tmp_path / "groups.txt"
    groups.write_text("sp|A|GENE_A tr|B|GENE_B\nA\n")
    monkeypatch.setattr(og_to_pairwise.sys, "argv", ["converter", str(groups), str(input_dir)])
    with pytest.raises(ValueError, match="occurs in multiple groups"):
        og_to_pairwise.main()

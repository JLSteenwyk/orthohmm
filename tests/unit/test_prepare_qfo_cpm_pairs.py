...
def accounting(state="COMPLETED", cpus="2", exit_code="0:0", node="bizon"):
    return ("JobID|JobIDRaw|State|ExitCode|NodeList|AllocCPUS\n"
            f"21976_0|21977|{state}|{exit_code}|{node}|{cpus}\n")
...
def test_scheduler_unique_array_identity():
    assert module.completed_admission(accounting(), 0)["JobIDRaw"] == "21977"
    with pytest.raises(ValueError):
...
def test_prepare_real_conversion_with_mocked_native_recheck(tmp_path, monkeypatch, problem):
    executor = tmp_path / "benchmarks/work/publication_qfo_cpm_phylogeny_admission_v1"
    ...
    admission = tmp_path / "benchmarks/work/qfo_cpm_native_admission_21976_0.json"

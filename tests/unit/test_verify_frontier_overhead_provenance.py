...
def fixture(index):
    ...
    runtime.append(dict(path=str(module.ROOT / "frontier_overhead_recipe_v1.json"), sha256=module.RECIPE_SHA,
                        records=41, scientific_execution_authorized=False, status="runtime_tree_identity_matches"))
    ...
    measured = dict(job_id=123, launched=["srun", "--exclusive", "--exact", "--nodes=1", "--ntasks=1", "--cpus-per-task=20",
        ...
        str(module.ROOT / "frontier_overhead_recipe_v1/benchmark_tools" / collector), "--worker", run["measurement_directory"]])
    ...
    receipt = dict(task=deepcopy(task), authorization=deepcopy(context["authorization"]),
                   authorization_sha256=module.AUTH_SHA, plan_sha256=module.PLAN_SHA, scientific_timings_admitted=False)
    scheduler = (f"JobId=123 ArrayJobId=21838 ArrayTaskId={index} JobState=COMPLETED ExitCode=0:0 Restarts=0 Requeue=0 "
                 "NodeList=spark-7ff0 OverSubscribe=NO MinMemoryNode=96G NumNodes=1 NumCPUs=20 CPUs/Task=20 "
                 f"Command={module.ROOT}/frontier_overhead_recipe_v1/benchmark_tools/run_dgx_frontier_overhead.sh")
    return [context, index, prepared, verified, receipt, measured, scheduler]


@pytest.mark.parametrize("index", range(18))
def test_all_tasks_bind_to_pinned_context(index):
    args = fixture(index)

"""Audit all three terminal dual-native diagnostics; never admit scientific timing."""
...
from benchmark_tools.verify_dual_native_provenance import load_context, verify, ROOT, JOBS
from benchmark_tools.replay_dual_native_measurement import replay
...
def successful_task(archive, context, task, scheduler, prior):
    ...
    preparation, verification, receipt = [json.loads((directory / name).read_text()) for name in (
        "preparation.json", "verification.json", "dual_diagnostic_task.json")]
    measured = json.loads((directory / "measurement/dual_bracket_report.json").read_text())
    ...
    return dict(index=task["index"], method=task["method"], job_id=binding["job_id"],
        ...
        boot_id=measured["points"][0]["frontier"]["boot_id"], inventory=snapshots,
...
def audit(archive, results, scheduler_directory, prior_path):
    ...
    sources = [record(results / name) for name in (
        "dgx_dual_native_plan_20260919.json", "dgx_dual_native_recipe_20260919.json",
        "dgx_pressure_overhead_plan_v2_20260919.json", "DUAL_BRACKET_NATIVE_PROTOCOL_20260919.md")]
    recipes = recipe_evidence(archive, context["recipe"], "dual_native_recipe_v1")
    ...
    for item in [*scheduler_sources, *sources, *recipes, prior_source]:
        check(item)
    return dict(status="dual_native_diagnostics_audited_not_scientific_admission", runs=rows,
        ...
        source=record(__file__), helpers=[record(Path(__file__).with_name(name)) for name in (
            "verify_dual_native_provenance.py", "replay_dual_native_measurement.py",
            "fingerprint_native_overhead_outputs.py", "validate_scaling_outputs.py",
            "measure_native_dual_bracket_step.py", "measure_native_frontier_step.py",
            "probe_dual_cpu_brackets.py", "audit_frontier_overhead.py", "capture_job_scheduler.py")],

"""Reuse frozen native smoke commands with a separate counter-only step collector."""
...
from measure_native_counter_step import measure
...
def relocate(value):
    old, new = str(ROOT / "launcher_smoke_v1"), str(ROOT / "counter_native_smoke_v1")
...
def launch(spec_path, index, recipe, recipe_sha):
    ...
    os.environ["PYTHONPYCACHEPREFIX"] = str(ROOT / "counter_native_smoke_v1" / f"cache_{index}")
    ...
    os.chdir(run["cwd"])
    result = measure_run(run, spec["orders"][0], runtime, native_enumerator(spec["enumerator"]),

...
from benchmark_tools.run_dgx_hierarchy_quiet_smoke import ROOT, SPEC_SHA, relocate, read_pinned
...
def audit(archive, spec_path):
    ...
    for i, original in enumerate(spec["runs"]):
        directory = archive / "hierarchy_quiet_smoke_v1" / f"run_{i:02d}"
    ...
    for item in inventory:
        check(item)
    return dict(status="three_hierarchy_quiet_smokes_validated_not_timing_admission", source=record(__file__),

...
PANELS = {"fixed_length_v1": (SEEDS, 20261031),
          "variable_length_v2": (tuple(range(20261101, 20261111)), 20261130)}
...
def paired_seed_summary(comparator, baseline, bootstrap_seed=20261031):
...
def summarize(records, panel="fixed_length_v1"):
    if panel not in PANELS:
        raise ValueError("Unknown frozen simulation panel")
    seeds, bootstrap_seed = PANELS[panel]
    index = {}
    for row in records:
    ...
    result = {"schema_version": 1, "publication_ready": False, "panel": panel, "statistic": "mean of seed-level metrics",
              "bootstrap": {"replicates": 20000, "seed": bootstrap_seed, "rng": "PCG64 multinomial",
    ...
    for condition in CONDITIONS:
        ...
        for method in METHODS[:2]:
            ...
            summary = paired_seed_summary({s: index[(condition, s, method)]["score"] for s in included},
                                          {s: index[(condition, s, "orthofinder_full")]["score"] for s in included}, bootstrap_seed)
...
def main():
    ...
    parser.add_argument("--panel", choices=sorted(PANELS), default="fixed_length_v1")
    args = parser.parse_args()
    if args.output.exists():
        raise FileExistsError("Refusing to overwrite analysis")
    data = args.records.read_bytes()
    result = summarize(json.loads(data)["records"], args.panel)

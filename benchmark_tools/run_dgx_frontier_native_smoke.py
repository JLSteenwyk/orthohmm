"""Frozen three-method native smoke with complete-command frontier observation."""
...
from measure_native_frontier_step import measure
...
def relocate(value):
    old, new = str(ROOT / "launcher_smoke_v1"), str(ROOT / "frontier_native_smoke_v1")
...
def launch(spec_path, index, recipe, recipe_sha):
    ...
    os.environ["PYTHONPYCACHEPREFIX"] = str(ROOT / "frontier_native_smoke_v1" / f"cache_{index}")

"""Complete-command paired-bracket engineering collector, not timing admission."""
...
def evaluate(points, done, job):
    original = original_evaluate(points, done, job)
    ...
    if [p["outer"] for p in pairs] != original["original_threshold_screen"]["intervals"]:
        raise ValueError("Dual outer intervals differ from original evaluator")
    narrow = [p["narrow"] for p in pairs]
    return dict(original_screening=original, narrow_intervals=narrow,
                narrow_flagged_intervals=[i for i, row in enumerate(narrow) if not row["screen_passed"]],
                ...
                limitations=["Observation-window endpoints bracket command and wrapper work; not exact command boundaries.",
                             "Narrow screen is additional diagnostic, not retrospective eligibility replacement.",
                             "Both residuals remain non-atomic; no foreign-CPU identification or timing correction."])
        ...
        try:
            ...
            while True:
                ...
                if process.poll() is not None:
                    raise RuntimeError("Worker exited before final dual observation")
            ...
            result = dict(status="command_exited_zero" if done["exit_code"] == 0 else "command_failed",
                ...
                limitations=["Complete-command dual-bracket engineering measurement, not controlled comparative timing.",
            ...
            save(directory / "dual_bracket_report.json", result)
            return result
        finally:

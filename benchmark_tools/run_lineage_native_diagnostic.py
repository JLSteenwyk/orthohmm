"""Derive and execute only the three frozen native aggregate-lineage diagnostics."""
...
from benchmark_tools.measure_native_lineage_step import measure
...
PROTOCOL_SHA = "7a20fe9e759e10c8e42c269d66775c28b1d3da763e8b73bdac9c18b4ee21d00b"
INDICES = (1, 3, 8)
...
def build(base_path, protocol_path):
    ...
    for index, (original, method) in enumerate(zip(INDICES, METHODS)):
        ...
        run = relocate(template["run"], str(ROOT / "pressure_frontier_overhead_v2" / f"run_{original:02d}"),
                       str(ROOT / "lineage_native_v1" / f"run_{index:02d}"))
    ...
    plan.update(status="prospective_lineage_native_diagnostic", runs=runs,
...
def launch(plan_path, plan_sha, recipe_path, recipe_sha, index):
    ...
    os.environ["PYTHONPYCACHEPREFIX"] = str(ROOT / "lineage_native_v1" / f"cache_{index}")
    ...
    with (Path(run["measurement_directory"]).parent / "lineage_diagnostic_task.json").open("x") as stream:

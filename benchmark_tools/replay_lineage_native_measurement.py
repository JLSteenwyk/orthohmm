"""Replay aggregate-lineage native evidence without admitting comparative timings."""

import json
import math
from pathlib import Path

from benchmark_tools.measure_native_lineage_step import evaluate, interval_point
...
def replay(directory, job_id, expected_command, expected_timeout_s=900):
    ...
    files = [directory / name for name in (
        "lineage_report.json", "command.json", "done.json", "step_memory.json")]
    ...
    return dict(status="lineage_native_measurement_replayed", native_wall_s=wall,

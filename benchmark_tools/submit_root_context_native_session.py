"""Hold one bounded submitting session for the frozen one-hour native panel."""
...
from benchmark_tools.verify_root_context_native_provenance import RECIPE_ROOT, SUBMISSION_SCRIPT
...
def run(output, recipe_sha):
    ...
    try:
        ...
        if queue.returncode != 0 or queue.stdout.strip():
            ...
            result.update(status="wait_returned", returncode=waited.returncode,
                stdout=waited.stdout, stderr=waited.stderr, job_id=job_id(waited.stdout),
        ...
        result.update(status="observation_timeout", stdout=decoded(error.stdout), stderr=decoded(error.stderr),
            ...
            error="Observation timeout is not terminal evidence; inspect the same Slurm job, never automatically resubmit.")

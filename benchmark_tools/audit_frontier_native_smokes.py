"""Replay complete-command frontier smokes and validate native outputs."""
...
from benchmark_tools.run_dgx_frontier_native_smoke import ROOT, SPEC_SHA, relocate, read_pinned
...
def audit(archive, spec_path):
    ...
    for i, original in enumerate(spec["runs"]):
        directory = archive / "frontier_native_smoke_v1" / f"run_{i:02d}"
        ...
        measured = json.loads((directory / "measurement/frontier_report.json").read_text())
    ...
    for item in inventory:
        check(item)
    return dict(status="three_frontier_native_smokes_validated_not_timing_admission", source=record(__file__),

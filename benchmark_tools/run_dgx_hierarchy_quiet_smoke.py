"""Frozen three-method native smoke with complete-command hierarchy observation."""
...
from measure_native_hierarchy_step import measure
...
def relocate(value):
    old, new = str(ROOT / "launcher_smoke_v1"), str(ROOT / "hierarchy_quiet_smoke_v1")
...
def launch(spec_path, index, recipe, recipe_sha):
    ...
    os.environ["PYTHONPYCACHEPREFIX"] = str(ROOT / "hierarchy_quiet_smoke_v1" / f"cache_{index}")

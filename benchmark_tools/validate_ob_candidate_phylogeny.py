"""Admit native outputs for a fixed candidate-threshold phylogeny variant."""
...
from benchmark_tools.run_ob_candidate_neighborhood import ADMISSION_SHA, VARIANTS, variant_cell
...
def completed_task(accounting, index):
    ...
    selected = [row for row in rows if row["JobID"] == f"21316_{index}"]
...
def check_execution(status, preflight, postflight, cell, arm, scheduler, index, launcher):
    ...
    expected = {"job_id": scheduler["JobIDRaw"], "array_job_id": "21316", "array_task_id": str(index),
...
def validate(root, index):
    ...
    expected = subprocess.check_output(["git", "-C", str(root), "rev-parse", "e8e86d8^{commit}"], text=True).strip()
    ...
    check_execution(status, preflight, postflight, cell, arm, scheduler, index, launcher)

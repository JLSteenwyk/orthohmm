...
recipe="$root/hierarchy_native_recipe_v1"
...
export PYTHONPYCACHEPREFIX="$root/hierarchy_native_smoke_v1/launcher_cache_$index"
test ! -e "$PYTHONPYCACHEPREFIX"
unset PYTHONPATH LD_PRELOAD LD_LIBRARY_PATH LD_AUDIT
exec "$root/envs/orthohmm/bin/python" -B "$recipe/benchmark_tools/run_dgx_hierarchy_native_smoke.py" \
  --spec "$recipe/benchmark_tools/dgx_launcher_smoke_spec_20260917.json" \
  --recipe "$root/hierarchy_native_recipe_v1.json" --recipe-sha "$sha" --index "$index"

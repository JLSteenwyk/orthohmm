"""Replay complete-command interval smokes and validate native outputs."""
...
from benchmark_tools.run_dgx_interval_native_smoke import ROOT, SPEC_SHA, relocate, read_pinned
...
def audit(archive, spec_path):
    ...
    for i, original in enumerate(spec["runs"]):
        directory = archive / "interval_native_smoke_v2" / f"run_{i:02d}"
        ...
        measured = json.loads((directory / "measurement/interval_report.json").read_text())
        ...
        if memory["errors"] or memory["scope"] != measured["points"][-1]["native_cpu_scope"]:
    ...
    for item in inventory:
        check(item)
    return dict(status="three_interval_native_smokes_validated_not_timing_admission", source=record(__file__),

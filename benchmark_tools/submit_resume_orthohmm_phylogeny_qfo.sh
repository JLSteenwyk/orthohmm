...
readonly OUTDIR=${OUTDIR:-${BASE}/qfo_benchmark/results/orthohmm_phylogeny_satellite_v2}
...
restart_job=$(sbatch --parsable \
    --job-name=qfo_ohmm_phylo_v2_restart \
    --time=7-00:00:00 \
    --cpus-per-task="${CPUS}" \
    --mem="${MEM}" \
    --output="${LOG_DIR}/qfo_orthohmm_phylogeny_v2_restart_%j.out" \
    --error="${LOG_DIR}/qfo_orthohmm_phylogeny_v2_restart_%j.err" \
...
scoring_job=$(sbatch --parsable \
    ...
    --job-name=qfo_score_ohmm_phylo_v2 \
    --time=7-00:00:00 \
    --cpus-per-task="${SCORE_CPUS}" \
    --mem="${SCORE_MEM}" \
    --output="${LOG_DIR}/qfo_score_orthohmm_phylogeny_v2_%j.out" \
    --error="${LOG_DIR}/qfo_score_orthohmm_phylogeny_v2_%j.err" \
    --export=ALL,REPO_ROOT="${BASE}",METHOD=orthohmm_phylogeny_satellite_v2,PAIRS="${OUTDIR}/pairs.qfo.tsv",QFO_WORK_ROOT="${QFO_WORK_ROOT}" \

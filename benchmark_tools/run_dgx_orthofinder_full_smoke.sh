#!/usr/bin/env bash
# Fresh full default pipeline, not a scientific timing run (strace is enabled).
set -euo pipefail
root=$(realpath "${1:?Usage: run_dgx_orthofinder_full_smoke.sh PROJECT_ROOT RECIPE_DIRECTORY}")
recipe=$(realpath "${2:?Require original recipe directory}")
input="$root/smoke_missing20_20261101_input"
output="$root/orthofinder_full_smoke_v1"
prefix="$root/envs/orthofinder"
test "$(uname -m)" = aarch64
test ! -e "$output"
test -d "$input"
export PATH="$prefix/bin:$root/diamond-2.0.13-build-v2:$root/mcl-prefix-v3/bin:$root/external-prefix-v1/fasttree-2.1.11:$root/external-prefix-v1/mafft-7.525/bin:$root/famsa-source-v1:$root/fastme-prefix-v1/bin:/usr/bin:/bin"
...
expected = {
    ...
    "fastme": "2ea7efd4b791cc9fd8b90fd6f5575815d41804268c98444b867c6bf698cf1be9",
}
...
strace -f -s 16384 -e trace=execve -o "$output/execve.log" \
    ...
    -o "$output/results" > "$output/native.log" 2>&1 || status=$?
...
test "$(find "$output/results" -name 'Orthologues' -type d | wc -l)" = 1

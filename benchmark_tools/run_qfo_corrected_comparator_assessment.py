...
from benchmark_tools.prepare_qfo_corrected_comparator_pairs import ENV_SHA, METHODS
from benchmark_tools.verify_ygob_validation import require_completed_job


def validate_stage(stage, method, scheduler):
    if (stage["status"] != "corrected_comparator_pairs_prepared_unscored" or stage["accuracy_evaluated"] is not False
    ...
    if (type(stage["total_pairs"]) is not int or stage["total_pairs"] <= 0

"""Boundary-only arm using the same lineage observations as the periodic collector."""
...
from benchmark_tools.measure_native_lineage_step import read_point, validate_point, interval_point
from benchmark_tools.probe_dgx_cpu_hierarchy import compare
from benchmark_tools.probe_cgroup_lineage import compare as compare_lineage
...
def evaluate(points, done, job):
    ...
    return dict(schema="lineage_boundary_screening_v1", whole_command_screen=whole,
        hierarchy_boundary=compare(points[0], points[1], job),
        lineage_boundary=compare_lineage(points[0]["lineage"], points[1]["lineage"]),
        native_pressure_whole_command=compare_pressure(points[0]["native_pressure"], points[1]["native_pressure"], job),
        ...
        limitations=["Boundary-only observations cannot detect localized competing activity.",
            "Same point reader, worker and completion polling as the periodic lineage arm.",
    ...
    with (directory / "step.log").open("x") as log:
        ...
        try:
            ...
            result = dict(status="command_exited_zero" if done["exit_code"] == 0 else "command_failed",
                ...
                limitations=["Boundary-only lineage engineering measurement, not controlled comparative timing.",
            ...
            save(directory / "lineage_boundary_report.json", result)
            return result
        finally:

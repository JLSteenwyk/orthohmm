...
| --- | --- | --- |
| 1. Frozen publication baseline | [Corrected-QfO table with seven admitted rows](qfo_corrected_comparison_20260923_v6/scores.md), scoring corrections and prospective method pin; original-release OrthoMCL final-group completion and failure audit are historical evidence only | Corrected OrthoMCL remains unadmitted. Its [frozen recovery workflow](QFO_RECOVERY_BPO_HANDOFF_20260923.md) is implemented and fixture-tested, not production-complete. Require actual search/BPO/native-group/conversion/score admissions, corrected failure-impact analysis and consolidated raw-output provenance for every retained row; exact commands/versions/resources remain part of the baseline |
...
| 4. Uncertainty and error explanation | Paired OrthoBench/SwissTrees intervals, factorial and sequence controls; domain/composition strata; descriptive [identity](CORRECTED_SWISS_IDENTITY_RESULT_22102.md), [historical fragment](SWISS_HISTORICAL_FRAGMENT_RESULT_20260923.md) and [validated retained-tree duplication](SWISS_DUPLICATION_FEATURE_RESULT_20260923.md) analyses; native GO/EC/FAS arithmetic and VGNC/TreeFam dependency audits; observed search/grouping traces | Appropriate uncertainty for other QfO endpoints/secondary mean remains unmet. Identity is not calibrated divergence; fragment unflagged is not proven complete. Duplication strata are reference-derived annotations, not independent evolutionary rates or causal evidence. Observed rejection localization does not prove causal mechanisms, historical raw-score identity, counterfactual recovery or independent FAS validation |

...
| Corrected recovered OrthoMCL final-group QfO scoring is complete | [Independent score admission](qfo_recovered_score_admission_22176.json), [corrected pair conversion](qfo_recovered_pairs_22172.json), [eight-method table](qfo_corrected_comparison_20260926_v7/scores.md) | Supported for the recovered native-representation run: 79,862 groups, 781,432 grouped proteins, 14,378,089 cross-species clique pairs and all six metrics admitted. SwissTrees F1 is 0.766610; project-defined secondary mean is 0.713526. Not an uninterrupted or repaired-query counterfactual |
| OrthoMCL BLAST failures have negligible impact | [Corrected membership audit](qfo_recovered_failure_membership_20260926.json), [corrected exposure](qfo_recovered_reference_impact_20260926.json) | Not established. All 53 failed queries are absent from final groups, including 21 with incoming hits. None directly occurs in SwissTrees/pooled TreeFam-A/VGNC references; four have experimental GO and 46 have FAS features. This does not bound indirect or repaired-search counterfactual effects |
| Legacy residue-deletion proteins contribute to submitted cross-species pairs | [Verified final-group trace](qfo_native_residue_group_trace_20260926.json) | Contradicted for the observed run: all seven occur in three single-species groups with zero incident cross-species pairs. A residue-preserving search counterfactual remains unexecuted |
...
| Corrected QfO accuracy or rankings are established | [Eight-method table](qfo_corrected_comparison_20260926_v7/scores.md), [complete comparator intervals](qfo_recovered_swiss_uncertainty_22178.json), [24-endpoint reproduction](qfo_recovered_swiss_reproduction_22178.json), [complete factorial](QFO_CORRECTED_FACTORIAL_COMPLETE_20260919.md) | Eight point-estimate rows and eight conditional SwissTrees contrasts are admitted. Full OrthoFinder has higher point estimates than phylogenetic OrthoHMM on all three F1 endpoints; OrthoHMM has higher GO/EC similarity and FAS. Corrected phylogenetic-minus-sensitive SwissTrees F1 is +0.148015 with adjusted interval [0.049791, 0.266015], but phylogenetic-minus-OrthoFinder intervals include zero. Recovered OrthoMCL minus full OrthoFinder F1 is -0.081804 with adjusted interval [-0.159935, -0.008177]. All seven earlier contrasts are unchanged; FastOMA still uses a supplied species tree. All eight factorial cells are separately complete. No general superiority, equivalence or complete ranking; historical results cannot be relabeled |
...
| Observed search rejections identify the cause of final OrthoBench F1 losses | [Raw search-decision recount](OB_SEARCH_DECISION_RESULT_20260918.md), [final-grouping join](OB_SEARCH_GROUPING_JOIN_20260918.md), [forced scoring](ob_forced_candidates_audit_20260926.json) | Final F1 causality remains unestablished: 8,386 pairs excluded in both directions nevertheless share final groups. Forced scoring now isolates a search-hit mechanism, not downstream grouping accuracy or historical raw-score equality |
| Prefilter selection excludes pairs that pass unchanged HMM scoring | [Protocol](OB_FORCED_CANDIDATE_PROTOCOL_20260926.md), [independent audit](ob_forced_candidates_audit_20260926.json), [figure](figures_forced_candidates_20260926/forced_candidates.pdf) | Supported for the reference-conditioned diagnostic: 30,496 of 48,368 excluded directed pairs pass E<1e-4 when forced; all 33,098 previously scored pairs retain exact scores/E-values/decisions. Not unbiased sensitivity, proof of orthology, matched DIAMOND sensitivity or a downstream F1 gain |
...
| 4. Uncertainty and error explanation | Paired OrthoBench/SwissTrees intervals, factorial and sequence controls; domain/composition strata; descriptive [identity](CORRECTED_SWISS_IDENTITY_RESULT_22102.md), [historical fragment](SWISS_HISTORICAL_FRAGMENT_RESULT_20260923.md) and [validated retained-tree duplication](SWISS_DUPLICATION_FEATURE_RESULT_20260923.md) analyses; native GO/EC/FAS arithmetic and VGNC/TreeFam dependency audits; observed search/grouping traces and [forced-scoring mechanism audit](ob_forced_candidates_audit_20260926.json) | Appropriate uncertainty for other QfO endpoints/secondary mean remains unmet. Identity is not calibrated divergence; fragment unflagged is not proven complete. Duplication annotations are not causal evidence. Forced scoring establishes 30,496 missed significant hits in the watched set, not unbiased sensitivity, downstream grouping/F1 effects, historical raw-score identity or independent FAS validation |
| 5. Robustness and practical efficiency | Corrected fixed/variable-length multi-seed simulations, complete generating-tree/NNI simulation controls, six OrthoBench tree perturbations and six parameter variants; all 27 historical DGX runs with native validation, resource replay, descriptive summaries and figure; [replacement protocol](DGX_SCALING_REPLACEMENT_PROTOCOL_20260920.md), [long-run amendment](DGX_SCALING_LONG_RUN_AMENDMENT_20260920.md) and tested measurement audit | Controlled resource comparison remains unproven. The user deferred further DGX work; the replacement panel is unrun and no dedicated-machine timing is currently authorized. Any resumption requires a frozen environmental policy, launch/session integration and complete native/environmental admission. Shared-host observations do not replace controlled timing. QfO robustness and broader evolutionary realism remain limited |

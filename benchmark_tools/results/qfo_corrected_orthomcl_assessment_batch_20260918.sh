#!/bin/bash
#SBATCH --job-name=qfo_corrected_orthomcl_assess
#SBATCH --nodelist=bizon
#SBATCH --cpus-per-task=8
#SBATCH --mem=64G
#SBATCH --time=24:00:00
#SBATCH --no-requeue
#SBATCH --output=/mnt/ca1e2e99-718e-417c-9ba6-62421455971a/ORTHOHMM/orthohmm/benchmarks/work/qfo_corrected_orthomcl_assess_%j.log
set -euo pipefail
...
exec /home/bizon/anaconda3/bin/python "$EXECUTOR/benchmark_tools/run_qfo_corrected_comparator_assessment.py" \
    --root "$ROOT" --method orthomcl --pairs-sha256 "$SHA" --conversion-job "$CONVERSION_JOB"

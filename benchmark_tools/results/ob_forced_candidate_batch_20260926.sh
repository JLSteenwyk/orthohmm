#!/bin/bash
#SBATCH --job-name=ob_forced_candidates
#SBATCH --nodelist=bizon
#SBATCH --cpus-per-task=4
#SBATCH --mem=16G
#SBATCH --time=12:00:00
#SBATCH --no-requeue
#SBATCH --output=/mnt/ca1e2e99-718e-417c-9ba6-62421455971a/ORTHOHMM/orthohmm/benchmarks/work/ob_forced_candidates_%j.log
set -euo pipefail
...
exec /home/bizon/anaconda3/bin/python -B "$EXECUTOR/benchmark_tools/trace_ob_search_decisions.py" \
  --root "$ROOT" --core "$ROOT/benchmarks/work/publication_method_native_v2" \
  --output "$ROOT/benchmarks/results/ob_forced_candidates_v1" --threads 4 --force-watched-candidates

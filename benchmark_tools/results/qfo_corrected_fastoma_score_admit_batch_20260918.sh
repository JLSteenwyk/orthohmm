#!/bin/bash
#SBATCH --job-name=qfo_corrected_fastoma_score_admit
#SBATCH --nodelist=bizon
#SBATCH --cpus-per-task=2
#SBATCH --mem=64G
#SBATCH --time=04:00:00
#SBATCH --no-requeue
#SBATCH --output=/mnt/ca1e2e99-718e-417c-9ba6-62421455971a/ORTHOHMM/orthohmm/benchmarks/work/qfo_corrected_fastoma_score_admit_%j.log
set -euo pipefail
...
exec /home/bizon/anaconda3/bin/python "$EXECUTOR/benchmark_tools/admit_qfo_corrected_comparator_assessment.py" \
    --root "$ROOT" --method fastoma --job "$JOB" --conversion-job "$CONVERSION_JOB" \
    --pairs-sha256 "$SHA" --output "$ROOT/benchmarks/work/qfo_corrected_fastoma_assessment_admission_20260918.json"

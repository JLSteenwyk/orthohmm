...
def verify(root, output):
    ...
    if admit_source["sha256"] != ADMIT_SHA or record(Path(__file__).with_name("admit_blast_recovery_batch.py"))["sha256"] != ADMIT_SHA:
        raise ValueError("Changed admission or coverage implementation")
    accounting = subprocess.check_output(["sacct", "-j", "22103,22105", "--parsable2",
        "--format=JobID,JobIDRaw,State,ExitCode,NodeList,AllocCPUS,Elapsed"], text=True)
    scheduler_rows = list(csv.DictReader(io.StringIO(accounting), delimiter="|"))
    ...
    for index, batch in enumerate(batches["batches"]):
        completed_task(accounting, index)
        validators = [r for r in scheduler_rows if r["JobID"] == f"22105_{index}"]
        if len(validators) != 1 or tuple(validators[0][k] for k in ("State", "ExitCode", "NodeList", "AllocCPUS")) != (
                "COMPLETED", "0:0", "bizon", "2"):
            raise ValueError("Require all twenty completed admission jobs")
        path = root / f"benchmarks/work/qfo_blast_recovery_admission_{index}_20260923.json"
        item = record(path)
        report = json.loads(path.read_text())
        if admit_source not in report["records"]:
            raise ValueError("Batch was not checked by the frozen admission source")
        if report["scheduler"] != completed_task(accounting, index):
        ...
        admissions.append(dict(index=index, report=item, scheduler=validators[0]))
...
if __name__ == "__main__":
    ...
    verify(args.root.resolve(), args.output.absolute())

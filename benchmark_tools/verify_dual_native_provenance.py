"""Bind dual-native diagnostics to pinned tasks, inputs, runtime and scheduler."""
...
from benchmark_tools.run_dual_native_diagnostic import ROOT, build, read_pinned

PLAN_SHA = "029f19b0e21f356387ae4e2df50310a225cdba4f356037814646af57226e1113"
RECIPE_SHA = "bea6cbee0a0a22c70a26b14eff137c65d95cd6d885f857d570dd11e515a34b46"
JOBS = (21912, 21913, 21914)
RECIPE_ROOT = ROOT / "dual_native_recipe_v1"
...
def load_context(results):
    ...
    expected = build(results / "dgx_pressure_overhead_plan_v2_20260919.json",
                     results / "DUAL_BRACKET_NATIVE_PROTOCOL_20260919.md")
    if not same(plan, expected):
        raise ValueError("Frozen plan differs from prescribed derivation")
    return dict(plan=plan, recipe=read_pinned(results / "dgx_dual_native_recipe_20260919.json", RECIPE_SHA))
...
def scheduler_identity(text, index):
    ...
    expected = dict(JobId=str(JOBS[index]), JobState="COMPLETED", ExitCode="0:0",
                    ...
                    Command=str(RECIPE_ROOT / "benchmark_tools/run_dgx_dual_native.sh"))
...
def verify(context, index, preparation, verification, receipt, measurement, scheduler):
    ...
    runtime.append(dict(path=str(ROOT / "dual_native_recipe_v1.json"), sha256=RECIPE_SHA,
    ...
    launched = ["srun", "--exclusive", "--exact", "--nodes=1", "--ntasks=1", "--cpus-per-task=20",
                plan["launcher_python"], "-B", str(RECIPE_ROOT / "benchmark_tools/measure_native_dual_bracket_step.py"),
    ...
    return dict(status="dual_native_provenance_bound", job_id=job, run=run, measured_argv=argv,
